...
int load_map_from_file(char* dest)
{
    //1. Datei zum lesen öffnen
    char filename[] = "hans.pgm";
    char format[500];
    int sizex,sizey,res;
    FILE * fp = fopen(filename, "rt");
    ...
    {
        fscanf(fp, "%s", format);
        printf("format 1. line: %s\n", format);
        fscanf(fp, "%d", &sizex);
        printf("y from 2. line: %d\n", sizex);
        fscanf(fp, "%d",&sizey);
        printf("x from 2. line: %d\n", sizey);
        fscanf(fp, "%d", &res);
        printf("resolution 2. line: %d\n", res);
        for(int i=0; i< MAP_SIZE_PIXELS*MAP_SIZE_PIXELS; i++)
        {
            //int temp;
          // if( (dest[i]=fgetc(fp))!= EOF)
            fscanf(fp, "%d", &dest[i]);
           // if(dest[i]!=0)
                //printf("%d ", dest[i]);
           //else
        ...
        printf("Stichprobe pos 100: %d\n", dest[100]);
        printf("Stichprobe pos 0: %d\n", dest[0]);
        printf("\n----------\n");
    }
  ...
  /*  char s[MAXLINE];
    ...
    //2. Datei auslesen und in dest speichern
    return 1;
}

int main( int argc, const char** argv )
{
    char * mapbytesxy = new char[MAP_SIZE_PIXELS * MAP_SIZE_PIXELS];

    load_map_from_file(mapbytesxy);

    URG04LX laser2(false);
    laser2.connect(DEVICE);
   ...
   /* for (int i=1;i<=10;i++)
    {
            ...
            unsigned int ndata = laser2.getScan(data);
    ...
    (SinglePositionSLAM*)new RMHC_SLAM(laser2, MAP_SIZE_PIXELS, MAP_SIZE_METERS, random_seed) :
    (SinglePositionSLAM*)new Deterministic_SLAM(laser2, MAP_SIZE_PIXELS, MAP_SIZE_METERS);

   // SinglePositionSLAM * slam2 = random_seed ?
   // (SinglePositionSLAM*)new RMHC_SLAM(laser2, MAP_SIZE_PIXELS, MAP_SIZE_METERS, random_seed) :
   // (SinglePositionSLAM*)new Deterministic_SLAM(laser2, MAP_SIZE_PIXELS, MAP_SIZE_METERS);
    // Report what we're doing
    int nscans = 10;//scans.size();
 ...
 //slam->setmap((unsigned char *)mapbytesxy);
    // Loop over scans
    for (int scanno=0; scanno<nscans; ++scanno)
    {
         unsigned int data[1000];
        int * lidar = scans[0/*scanno*/];
        laser2.getScan(data);
        //printf("scanno: %d \n",scanno);
        // Update with/out odometry
        if (use_odometry)
        {
            Velocities velocities = robot.computeVelocities(odometries[0/*scanno*/], velocities);
            slam->update(/*lidar*/(int*)data, velocities);
        }
        else
        {
            slam->update(/*lidar/*/(int*) data);
        }
